...
{                                                       }
{*******************************************************}
// 27.8.26 Modified to fix Sonomo display scaling problem
...
type
  ...
  private
    ...
    function GetCurrentMetalDrawable: CAMetalDrawable;
  protected
    function GetScale: Single; override;
  public
    constructor Create(const AHandle: TOCLocal);
    destructor Destroy; override;
    class function FindForm(const AWindow: NSWindow): TCommonCustomForm;
    property Wnd: NSWindow read GetWindow;
    property View: NSView read GetView;
    /// <summary>Returns current Metal drawable, which is used for rendering at this moment.</summary>
    /// <remarks>Returns nil, if Metal is not used as Canvas or we are not in rendering cycle.</remarks>
    property CurrentMetalDrawable: CAMetalDrawable read GetCurrentMetalDrawable;
...
uses
  System.RTLConsts, System.SysUtils, System.SyncObjs, System.Rtti, System.Math, System.Messaging, System.DateUtils,
  ...
  FMX.Platform.SaveState.Mac, FMX.Platform.Metrics, FMX.Platform.Metrics.Mac, FMX.Platform.Timer.Mac,
  FMX.Platform.Logger.Mac, FMX.FontManager, FMX.FontManager.Mac, FMX.Printer;
...
begin
  ...
  {$REGION 'shift state'}
    ...
    if (Self.EventKind in [TEventKind.MouseDown, TEventKind.MouseMove, TEventKind.MouseUp]) and (AEvent.clickCount > 1) then
...
type
  ...
  IFMXApplicationDelegate = interface(NSApplicationDelegate)
   ['{A54E08CA-77CC-4F22-B6D9-833DD6AB696D}']
    ...
    function applicationSupportsSecureRestorableState(app: NSApplication): Boolean; cdecl; // macOS 12.0+
  end;
...
{$M+}
  ...
  MacMetalView = interface(NSView)
    ...
    procedure viewDidChangeBackingProperties; cdecl;
  end;

  CALayerDelegate = interface(IObjectiveC)
  ['{96CEFFF4-174E-442A-B759-8D209704A64E}']
    procedure displayLayer(layer: CALayer); cdecl;
  end;

  TMacMetalView = class(TFMXViewBase, NSTextInputClient, CALayerDelegate)
  private
    FMetalLayer: CAMetalLayer;
    FCurrentDrawable: CAMetalDrawable;
    procedure UpdateLayerSize;
    procedure UpdateBackingScaleFactorForLayer;
  protected
    function GetObjectiveCClass: PTypeInfo; override;
    function ConvertToBacking(const ASize: CGSize): CGSize;
  public
    constructor Create(const AOwner: TFMXWindow; const AFrameRect: NSRect);
    destructor Destroy; override;
    { CALayerDelegate }
    procedure displayLayer(layer: CALayer); cdecl;
    { NSView }
    ...
    procedure viewDidChangeBackingProperties; cdecl;
  public
    property CurrentDrawable: CAMetalDrawable read FCurrentDrawable;
  end;
  ...
  public
    ...
    property View: NSView read GetView;
    property FMXView: TFMXViewBase read FViewObj;
...
function DefaultNotificationCenter: NSNotificationCenter; inline;
begin
...
begin
  ...
  TPlatformServices.Current.AddPlatformService(IFMXPlatformPropertiesService, MetricsService);
  ...
  TPlatformServices.Current.AddPlatformService(IFMXFontManagerService, TMacFontManager.Create);
end;
...
function TFMXApplicationDelegate.applicationSupportsSecureRestorableState(app: NSApplication): Boolean;
begin
  Result := True;
end;
...
procedure TPlatformCocoa.MouseEvent(const Event: NSEvent);
  ...
  function IsTheOurWindow(const theEvent: NSEvent): Boolean;
  var
    WindowPoint: NSPoint;
    ScreenPos: NSPoint;
    Window: NSWindow;
  begin
    ...
    ScreenPos := Window.convertBaseToScreen(WindowPoint);

    Result := TNSWindow.OCClass.windowNumberAtPoint(ScreenPos, 0) = Window.windowNumber;
  end;

var
  LEventRec: TEventRec;
begin
  if not IsTheOurWindow(Event) then
    Exit;
...
begin
  ...
  DefaultNotificationCenter.addObserver(GetObjectID, sel_getUid('frameChanged:'), NSStringToID(StrToNSStr('NSViewFrameDidChangeNotification')), GetObjectID);
  ...
  begin
    ...
    DefaultNotificationCenter.addObserver(GetObjectID, sel_getUid('surfaceNeedsUpdate:'), NSStringToID(NSViewGlobalFrameDidChangeNotification), GetObjectID);
  end;
end;
...
function TMacMetalView.GetObjectiveCClass: PTypeInfo;
begin
  Result := TypeInfo(MacMetalView);
end;

function TMacMetalView.makeBackingLayer: CALayer;
begin
  FMetalLayer := TCAMetalLayer.Wrap(TCAMetalLayer.OCClass.layer);
  FMetalLayer.setPixelFormat(PixelFormatToMTLPixelFormat(TCustomContextMetal.pixelFormat));
  FMetalLayer.setOpaque(not FOwner.Wnd.Transparency);
  FMetalLayer.setDevice(TCustomContextMetal.SharedDevice);
  FMetalLayer.setAllowsNextDrawableTimeout(False);
  FMetalLayer.setAutoresizingMask(kCALayerHeightSizable or kCALayerWidthSizable);
  FMetalLayer.setNeedsDisplayOnBoundsChange(True);
  FMetalLayer.setPresentsWithTransaction(True);
  FMetalLayer.setDelegate(GetObjectID);

  Result := FMetalLayer;
end;

procedure TMacMetalView.setFrameSize(newSize: NSSize);
begin
  NSView(Super).setFrameSize(newSize);
  UpdateLayerSize;
end;

procedure TMacMetalView.UpdateBackingScaleFactorForLayer;
var
  Window: NSWindow;
begin
  Window := NativeView.window;
  if (Window = nil) or (FMetalLayer = nil) then
    Exit;

  FMetalLayer.setContentsScale(Window.backingScaleFactor);
end;

procedure TMacMetalView.UpdateLayerSize;
var
  Window: NSWindow;
  NewSize: NSSize;
begin
  ...
  NewSize := NativeView.bounds.size;
  FMetalLayer.setDrawableSize(ConvertToBacking(NewSize));
end;

procedure TMacMetalView.viewDidChangeBackingProperties;
begin
  UpdateBackingScaleFactorForLayer;
  UpdateLayerSize;
end;

function TMacMetalView.ConvertToBacking(const ASize: CGSize): CGSize;
begin
  Result := NSView(Super).convertRectToBacking(CGRectMake(0, 0, ASize.width, ASize.height)).size;
end;

constructor TMacMetalView.Create(const AOwner: TFMXWindow; const AFrameRect: NSRect);
var
  V: Pointer;
begin
  inherited Create(AOwner);
  ...
  NSView(Super).setWantsLayer(True);
  NSView(Super).setAcceptsTouchEvents(True);
  NSView(Super).setLayerContentsRedrawPolicy(NSViewLayerContentsRedrawDuringViewResize or NSViewLayerContentsRedrawOnSetNeedsDisplay);
  NSView(Super).setLayerContentsPlacement(NSViewLayerContentsPlacementScaleAxesIndependently);
  NSView(Super).setNeedsDisplay(True);
  NSView(Super).setPostsFrameChangedNotifications(True);

  DefaultNotificationCenter.addObserver(GetObjectID, sel_getUid('frameChanged:'), NSStringToID(StrToNSStr('NSViewFrameDidChangeNotification')), GetObjectID);
end;

destructor TMacMetalView.Destroy;
begin
  DefaultNotificationCenter.removeObserver(GetObjectID);
  inherited;
end;

procedure TMacMetalView.displayLayer(layer: CALayer);
begin
  FCurrentDrawable := FMetalLayer.nextDrawable;
  if FCurrentDrawable <> nil then
  try
    drawRect(layer.bounds);
    FCurrentDrawable.present;
  finally
...
procedure TTextServiceCocoa.UpdateSelection;
var
  TextInput: ITextInput;
  SelRect: TRect;
begin
  ...
  begin
    ...
    begin
      FSelectedRange.location := SelRect.Right;
      FSelectedRange.length := Abs(SelRect.Width);
    end
    else
    begin
      FSelectedRange.location := SelRect.Left;
      FSelectedRange.length := SelRect.Width;
    end;
  end
  else
  begin
    FSelectedRange.location := CaretPosition.X;
    FSelectedRange.length := 1;
  end;
end;
...
procedure TTextServiceCocoa.CalculateSelectionBounds(out ASelectionStart, ASelectionEnd: Integer);
var
  TextInput: ITextInput;
  SelBounds: TRect;
...
procedure TFMXViewBase.DispatchKeyPress(const ACh: Char; const AKey: Word; const AShift: TShiftState);
var
  Key: Word;
  Ch: WideChar;
begin
  Key := AKey;
  Ch := ACh;
  DoKeyDown(Self, FOwner.Wnd, Key, Ch, AShift);
  DoKeyUp(Self, FOwner.Wnd, Key, Ch, AShift);
end;
...
var
  ...
  begin
    ...
    begin
      ...
      for I := ASelEnd to replacementRange.location + replacementRange.length - 1 do
        DispatchKeyPress(#0, vkDelete, []);
      for I := ASelStart - 1 downto replacementRange.location do
        DispatchKeyPress(#0, vkBack, []);
    end
    else if replacementRange.length > 0 then
    begin
      ATextSelection.SetSelection(TCaretPosition.Create(SavedCaretPosition.Line, replacementRange.location), replacementRange.length);
      DispatchKeyPress(#0, vkDelete, []);
    end;
  end;
  ...
  begin
    ...
    for I := RightPos to replacementRange.location + ReplacementRange.length - 1 do
      DispatchKeyPress(#0, vkDelete, []);
    for I := LeftPos - 1 downto ReplacementRange.location do
      DispatchKeyPress(#0, vkBack, []);
  end;
...
begin
  ...
  TextService := FocusedTextService;
  if TextService <> nil then
  begin
    TextService.CalculateSelectionBounds(SelStart, SelEnd);
    SavedCaretPosition := TextService.CaretPosition;
  end;
  ...
  try
    ...
    begin
      ...
      if TextService <> nil then
      try
        TextService.InternalSetMarkedText('');
        TextService.InternalEndIMEInput;
      except
        HandleException(Self);
      end;
    end;
    ...
    // If there are insert with replacement, we have to remove selected text from our TextInput control,
    // We emulate removing this part via Selection with Backspace key.
    if (TextService <> nil) and (replacementRange.location <> NSNotFound) then
    begin
      if Supports(FOwner.Wnd.Focused, ITextSelection, TextSelection) then
    ...
    begin
      ...
      begin
        Ch := Str.Chars[I];
        DispatchKeyPress(Ch, Ord(Ch), FShift);
      end;
...
begin
  ...
  begin
    Result.location := TextService.CaretPosition.X;
    Result.length := 0;
  end;
end;
...
begin
  Result := (Wnd <> nil) and (Wnd.Handle <> nil) and (Wnd.FormStyle <> TFormStyle.Popup);
end;
...
begin
  ...
  try
    ...
    if (ModWin <> nil) and (FWindow <> nil) and
      (NSObjectToID(ModWin) = (FWindow.Super as ILocalObject).GetObjectID) then
      NSApp.abortModal;
...
begin
  ...
  try
    ...
    if TOSVersion.Check(10, 12) then
      NSWin.setTabbingMode(NSWindowTabbingModeDisallowed);
    ...
    if GlobalUseMetal then
      FMXWin.FViewObj := TMacMetalView.Create(FMXWin, R)
    else
...
begin
  if GlobalUseMetal and not TCustomContextMetal.IsMetalSupported then
...
const
  SynchronizationPeriod = 50; // msec

  procedure CloseModalForm(const AForm: TCommonCustomForm);
  var
    CloseRes: TCloseAction;
  begin
    CloseRes := AForm.CloseModal;
    if CloseRes in [TCloseAction.caHide, TCloseAction.caMinimize] then
      AForm.Hide;
...
var
  ...
  NSWin: NSWindow;
  AutoReleasePool: NSAutoreleasePool;
  SavedStyleMask: NSUInteger;
  LastSynchronizationTime: TDateTime;
begin
  ...
  try
    ...
    NSWin := WindowHandleToPlatform(AForm.Handle).Wnd;
    SavedStyleMask := NSWin.styleMask;
    try
      // Disable Minimization for Modal form  
      NSWin.setStyleMask(SavedStyleMask - SavedStyleMask and NSMiniaturizableWindowMask);
      AForm.Show;
      ...
      try
        ...
        begin
          ...
          begin
            // This branch is called when the window is closed using the system buttons or Mac api.
            ...
            try
              if MR <> NSRunStoppedResponse then
                CloseModalForm(AForm);
            finally
              FModalStack.Pop;
            end;
          ...
          begin
            // The form could be closed programmatically.
            CloseModalForm(AForm);
            if AForm.ModalResult <> mrNone then
            begin
              NSApp.stopModal;
          ...
          HookObserverCallback(False);
          // We have to make synchronization, because modal form start inner loop, which doesn't account CheckSynchronize.
          if MilliSecondsBetween(LastSynchronizationTime, Now) > SynchronizationPeriod then
          begin
            CheckSynchronize;
            LastSynchronizationTime := Now;
          end;
      ...
      NSWin.setStyleMask(SavedStyleMask);
    end;
  finally
    AutoReleasePool.release;
  end;
...
type
  UTTypeClass = interface(NSObjectClass)
    ['{1CB37D97-0015-4FA4-83CB-909C08470553}']
  end;
  UTType = interface(NSObject)
  ['{788FE936-3B51-4DC8-A82E-6D1AB10178B0}']
  end;
  ...
  NSOpenSavePanelDelegate = interface(IObjectiveC)
    ...
    [MethodName('panel:didSelectType:')]
    procedure panelDidSelectType(sender: id; didSelectType: UTType); cdecl;
  end;

  TMacOpenSavePanelDelegate = class(TOCLocal, NSOpenSavePanelDelegate)
  private
    FDialog: TOpenDialog;
    FFiles: TStrings;
    FFilter: NSArray;
  public
    constructor Create(const ADialog: TOpenDialog; const AFiles: TStrings);
    ...
    procedure panelDidSelectType(sender: id; didSelectType: UTType); cdecl;
  public
    property Filter: NSArray read FFilter write FFilter;
  end;

{ TMacOpenSavePanelDelegate }

constructor TMacOpenSavePanelDelegate.Create(const ADialog: TOpenDialog; const AFiles: TStrings);
begin
  inherited Create;
  FDialog := ADialog;
  FFiles := AFiles;
...
procedure TMacOpenSavePanelDelegate.panelDidChangeToDirectoryURL(sender: Pointer; didChangeToDirectoryURL: NSURL);
begin
  if TNSNull.OCClass.null = NSObjectToId(didChangeToDirectoryURL) then
    FDialog.FileName := string.Empty
  else
    FDialog.FileName := NSStrToStr(didChangeToDirectoryURL.relativePath);
  if Assigned(FDialog.OnFolderChange) then
    FDialog.OnFolderChange(FDialog);
end;

procedure TMacOpenSavePanelDelegate.panelDidSelectType(sender: id; didSelectType: UTType);
begin
  if FFilter <> nil then
  begin
    FDialog.FilterIndex := Integer(FFilter.indexOfObject(NSObjectToId(didSelectType)));
    if Assigned(FDialog.OnTypeChange) then
      FDialog.OnTypeChange(FDialog);
  end;
end;

procedure TMacOpenSavePanelDelegate.panelSelectionDidChange(sender: Pointer);
var
  I: Integer;
begin
  FFiles.Clear;

  if NSObjectIsClass(Sender, 'NSOpenPanel') then
  begin
    var OpenPanel := TNSOpenPanel.Wrap(Sender);
    for I := 0 to Integer(OpenPanel.URLs.count) - 1 do
      FFiles.Add(NSStrToStr(TNSUrl.Wrap(OpenPanel.URLs.objectAtIndex(I)).relativePath));
  end
  else if NSObjectIsClass(Sender, 'NSSavePanel') then
  begin
    var SavePanel := TNSSavePanel.Wrap(Sender);
    FFiles.Add(NSStrToStr(SavePanel.URL.relativePath));
  end;

  if FFiles.Count > 0 then
    FDialog.FileName := FFiles[0];

  if Assigned(FDialog.OnSelectionChange) then
    FDialog.OnSelectionChange(FDialog);
end;
...
var
  ...
  I: Integer;
  AutoReleasePool: NSAutoreleasePool;
  Delegate: TMacOpenSavePanelDelegate;
...
begin
  ...
  try
    ...
    Delegate := TMacOpenSavePanelDelegate.Create(ADialog, AFiles);
    try
      OpenFile.setDelegate(Delegate.GetObjectID);
...
function TPlatformCocoa.DialogPrint(var ACollate, APrintToFile: Boolean;
  ...
  procedure SetActivePrinter(const APrinter: NSPrinter);
  var
    Device: TPrinterDevice;
    SelectedPrinterName: string;
    PrinterName: string;
  begin
    // NSPrinter doesn't provide unique printer ID and doesn't return PMPrinter, so we are not able to find exactly
    // the same printer and we are using Name. So if two printer use the same name, we will pick first.
    SelectedPrinterName := NSStrToStr(APrinter.name);
    for var I := 0 to Printer.Count - 1 do
    begin
      Device := Printer.Printers[I];
      PrinterName := Device.Title; // Title = Name on macOS
      if PrinterName = SelectedPrinterName then
      begin
        Printer.ActivePrinter := Device;
        Break;
      end;
    end;
  end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        SetActivePrinter(printInfo.printer);
        Result := True;
      end;
    finally
      printPanel.release;
...
begin
  ...
  try
    ...
    Delegate := TMacOpenSavePanelDelegate.Create(ADialog, AFiles);
    try
      SaveFile.setDelegate(Delegate.GetObjectID);
...
function TMacWindowHandle.GetCurrentMetalDrawable: CAMetalDrawable;
var
  FMXView: TFMXViewBase;
begin
  FMXView := TFMXWindow(FHandle).FMXView;

  if FMXView is TMacMetalView then
    Result := TMacMetalView(FMXView).CurrentDrawable
  else
    Result := nil;
end;
...
begin
// 27.08.26 Modified to fix Sonomo display scaling problem
...
begin
  ...
  begin
    ...
    { Copy from Context }
    ContextObject.Context.CopyToBits(FBits, FBufferSize.Width * 4, TRect.Create(TPoint.Zero, FBufferSize.Width, FBufferSize.Height));
    { Draw }
...
class function TMacWindowHandle.FindForm(const AWindow: NSWindow): TCommonCustomForm;
var
  I: Integer;
  AutoReleasePool: NSAutoreleasePool;
begin
  ...
  try
    ...
    for I := 0 to FWindowHandles.Count - 1 do
      if NSObjectToID(FWindowHandles[I].Wnd) = NSObjectToID(AWindow) then
      begin
